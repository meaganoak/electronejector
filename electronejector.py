...
good_gif = gifpy.load("yes.gif", loops=0)
bad_gif = gifpy.load("nope.gif", loops=0)

def iter_scaled_frames(gif):
    for frame, duration in gif.frames:
        scaled = pygame.transform.scale(frame, (screen_w,screen_h))
        yield scaled, duration


good_fullscreen_gif = iter_scaled_frames(good_gif)
bad_fullscreen_gif = iter_scaled_frames(bad_gif)
...
def eject(fullscreen_gif):
    ...
    for frame, duration in fullscreen_gif:
